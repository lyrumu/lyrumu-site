...
## 2026-09-25 · 封面底色与内页一致

- 移除首页独立渐变、纸纹层及浅色 body 底色覆写，封面直接显示与内页相同的主题底色；浅色为白色，暗色沿用现有深色。
- 内页背景与全局模板保持原实现，未添加公共背景层；同步更新 `PROJECT_MAP.md`。卡片环绕排版和统计降权保留。
- Hugo 生产构建（57 页）、SEO 回归（44 页）、首页与内页背景类检查及 `git diff --check` 通过；生成路径和 sitemap 不变。既有主题兼容警告仍在，未做浏览器验收或部署。

## 2026-09-25 · 提升首页圆盘环绕卡片的可读性

- 桌面三卡继续围绕唱片排列，侧卡取消倾斜并提高显示比例；卡片说明字号增大，移除编号、细分标签和重复装饰，改为直述各栏目内容。
- 封面统计保留原数据和逻辑，降低对比度与数字强调；`PROJECT_MAP.md` 更新首页入口维护字段。
- Hugo 生产构建（57 页）、生成页面入口检查和 `git diff --check` 通过；未部署，浏览器视觉验收和真实访客行为仍待确认。
